...
must_haves:
  truths:
    - "Project creator can view settings page"
    - "Project creator can rename the project"
    - "Project creator can add workspace members to the project"
    - "Project creator can remove members from the project"
    - "Project creator can delete the project"
    - "Non-creator members cannot access settings or see management controls"
    - "Members cannot remove themselves from the project"
  artifacts:
...
Purpose: Enable project creators (admins) to manage their projects - add/remove members, rename, and delete. This completes the full CRUD cycle for projects.
      ...
      <section className="mb-8">
        ...
        <div className="space-y-2">
          {projectMembers.map((member) => {
            ...
            // 3. Cannot remove yourself (members cannot remove themselves)
...
Key points:
- Fetches currentUser to determine if they are the project creator
- Uses `isCreator` flag (currentUser._id === project.creatorId) for all authorization checks
- Edit controls disabled for non-creators
- Add member section only visible to creator
- Remove member button logic:
  - Only shown if current user is creator (isCreator)
  - NOT shown if member is the creator (member.isCreator)
  - NOT shown if member is the current user (member.userId !== currentUser._id) - per CONTEXT.md "Members cannot remove themselves"
...
- All mutations use proper error handling with toast
  ...
  <verify>
    ...
    Test: as creator - rename project, change color, add/remove member, delete project
    Test: as non-creator member - controls should be disabled/hidden
  </verify>
  <done>
    ProjectSettings.tsx has working name/color editing (creator only)
    Member list displays with add/remove functionality (creator only)
    Remove button hidden for: non-creators, the creator member, and the current user (self)
    Delete button with confirmation (creator only) deletes project and navigates away
...
- Access control: only creator can manage, members cannot remove themselves
  </what-built>
  <how-to-verify>
...
7. **Test access control:**
   - As the creator, you should see all controls
   - Remove button should NOT appear next to your own name in member list
   - Remove button should NOT appear next to the creator's name
   - (If testing with a second non-creator user: they should see disabled controls)
...
5. Authorization enforced: only workspace admins create, only project creator manages
6. Members cannot remove themselves from projects
</verification>

<success_criteria>
Requirements verification:
- PROJ-01: User can create a project within a workspace (via sidebar dialog)
- PROJ-02: User can rename and delete projects they created (via settings page)
...
- Access control: creator-only management, members cannot remove themselves
</success_criteria>
