...
must_haves:
  ...
  artifacts:
    ...
    - path: "convex/tasks.ts"
      provides: "Internal getById query for token verification + clearDescription mutation"
  ...
  key_links:
    - from: "convex/collaboration.ts"
      to: "convex/tasks.ts"
      via: "internal.tasks.getById query for permission check"
      pattern: "internal\\.tasks\\.getById"
...
<task type="auto">
  ...
  <files>convex/collaboration.ts, convex/tasks.ts, src/hooks/use-yjs-provider.ts</files>
  <action>
1. In `convex/tasks.ts`, add an internal query `getById` that retrieves a task by ID (used by collaboration token verification). Use `internalQuery` from `./_generated/server`:
   ```typescript
   ...
   ```
   Import `internalQuery` from `./_generated/server` (add to existing import).

2. In `convex/collaboration.ts`:
...
3. In `src/hooks/use-yjs-provider.ts`, update the `ResourceType` type from `"doc" | "diagram"` to `"doc" | "diagram" | "task"`.
