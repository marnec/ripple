...
must_haves:
  truths:
    ...
    - "Editor content loads immediately from local cache when offline, without waiting for PartyKit"
  artifacts:
...
1. **Remove the `if (!provider) return` guard from the IndexedDB useEffect.** This is the critical decoupling step. Currently IndexedDB is only set up when `provider` is truthy (`if (!provider) return;`). Remove this guard entirely so IndexedDB persistence initializes based on `yDoc` and `documentId` alone, independent of provider. The IndexedDB persistence just needs the Y.Doc — it does not need the provider.

2. Move the IndexedDB `useEffect` to depend on `[documentId, resourceType, yDoc]` instead of `[provider, documentId, resourceType, yDoc]`. The guard removal in step 1 and this dependency change together ensure IndexedDB initializes immediately on mount, before the provider connects.
...
1. **Remove the `if (!provider) return` guard from the IndexedDB useEffect** to decouple IndexedDB initialization from provider. Depend on `[diagramId, yDoc]` instead of `[provider, diagramId, yDoc]`. This mirrors the useDocumentCollaboration change — IndexedDB initializes on mount without waiting for provider.
