...
must_haves:
  truths:
    ...
    - "users.getByIds query exists for batch-fetching user data for tooltips"
  artifacts:
...
- `by_message` → `["messageId"]` — fetch all reactions for a message
- `by_message_emoji_user` → `["messageId", "emoji", "userId"]` — uniqueness check for toggle idempotency
...
- Handler:
  ...
  3. If exists → `ctx.db.delete(existing._id)` (remove reaction)
  4. If not exists → `ctx.db.insert("messageReactions", { messageId, userId, emoji, emojiNative })` (add reaction)
  5. Return null

**2. `listForMessage` query:**
- Args: `{ messageId: v.id("messages") }`
- Returns: `v.any()` (complex enriched return type — use v.any() per project convention for enriched returns)
- Handler:
...
Do NOT use `filter()` — always use `withIndex()`.
  </action>
  <verify>Run `npm run lint` — should pass with zero warnings. Run `npx convex dev --once` to confirm functions deploy without errors.</verify>
