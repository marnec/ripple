...
export default defineSchema({
  ...
  channels: defineTable({
    ...
    //
    // These were `v.optional` for one release, because `convex deploy` pushes
    // the schema before `migrations:runAll` runs and Convex validates existing
    // documents at push time — so the migration that fills these columns had to
    // ship, and run, before a schema requiring them could be pushed. It has;
    // production was verified to have no row missing either column and none
    // still carrying `type`. `backfillChannelKindVisibility` and
    // `stripChannelType` stay in `runAll` as the repair path for a restored
    // backup.
