...
export function PwaUpdateProvider({ children }: { children: React.ReactNode }) {
  ...
  // In dev mode the SW is constantly regenerated by HMR, making needRefresh
  // permanently true. Only surface update state in production builds.
  const effectiveNeedRefresh = !import.meta.env.DEV && needRefresh;

  return (
    <PwaUpdateContext.Provider value={{ needRefresh: effectiveNeedRefresh, updateAndReload, checkForUpdate }}>
      {children}
